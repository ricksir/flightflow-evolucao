...
- fechado tecnicamente o ciclo pós-aceitação dos PRs **#36–#41**: scroll proporcional da Rota Processada sem término visual precoce, correções de sobreposição e destaque quente de campos atualizados, Mission Rail V9 recolhível com ganho real de área útil, compactação do shell superior, Temporal Deck inferior sem caixotes internos e estabilização do contrato de playback; baseline funcional `fc30aff8ffba37ecae94c9fc4810706071f79b77`, quality gate pós-merge **#146**, **699/699 Node + 115/115 Playwright**, com zero `failed`/`flaky`/`retry`/`timeout`/`AssertionError`/`not ok`/`SPATIAL_EQ_DIAG`, housekeeping **#43** concluído, nenhum PR aberto e apenas `main` + `design/pilot-shell-v1` remanescentes; permanece pendente apenas a revalidação humana final antes de release, sem abertura automática de V12;
