...
test('fechamento técnico atual preserva histórico V11 e registra certificação pós-PR34', () => {
  ...
  assert.ok(README.includes('PR31'));
  assert.ok(README.includes('PR34'));
});

test('fechamento técnico não declara revalidação humana inexistente', () => {
  assert.ok(README.includes('revalidação final pós-PR34 pendente'));
...
test('aceitação manual cobre breakpoints, fluxos protegidos e regressões pós-PR34', () => {
