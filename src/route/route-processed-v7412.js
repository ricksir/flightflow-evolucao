(() => {
  ...
  function renderModal(refreshRange=true) {
    ...
    qs('#ffrpTitle').textContent=`${model.history.callsign||'Plano'} · ${model.history.adep||'????'} → ${model.history.ades||'????'}`;
    qs('#ffrpSubtitle').textContent=snap.jurisdictionBoundaryFallback
      ? `${model.sourceFile||'Histórico'} · trecho APP operacional ${model.history.adep||'????'} → ${snap.appExitFix||snap.points.at(-1)?.ident||'????'} · ADES do plano ${model.history.ades||'????'} · rota declarada sem quadro PONTOS/ETIM · ${model.history.route||'—'}`
      : snap.declaredFallback
        ? `${model.sourceFile||'Histórico'} · rota declarada sem quadro PONTOS/ETIM · ${model.history.route||'—'}`
        : `${model.sourceFile||'Histórico'} · ${model.history.snapshots.length} quadro(s) de PONTOS · rota declarada: ${model.history.route||'—'}`;
    qs('#ffrpIdPlano').innerHTML=renderIdPlanoMarkup(model.history.idPlano);qs('#ffrpSnapTitle').textContent=`Quadro ${idx+1}/${model.resolvedSnapshots.length} · ${snap.operation||'rota processada'}`;
    ...
    renderMap(snap,progress);const range=qs('#ffrpRange');if(range){range.max=String(Math.max(0,Math.round(playbackLimit*1000)));range.value=String(Math.min(Number(range.max),Math.round(progress*1000)));}const freezeIndex=Number(profile?.jurisdictionEndIndex);const frozenDerived=derivedMode&&Number.isInteger(freezeIndex)&&freezeIndex>=0&&nei>=freezeIndex&&!terminal.active;qs('#ffrpTime').textContent=terminal.active?`${Math.round(progress*100)}% · fechamento terminal / Ordem TER · quadro ${idx+1}/${model.resolvedSnapshots.length}`:frozenDerived?`${Math.round(progress*100)}% · posição congelada no TER · sem ETIM histórico · quadro ${idx+1}/${model.resolvedSnapshots.length}`:derivedMode?`${Math.round(progress*100)}% · posição derivada por DEP + rota/velocidade · sem ETIM histórico · quadro ${idx+1}/${model.resolvedSnapshots.length}`:snap.declaredFallback?`${Math.round(progress*100)}% · rota declarada · sem ETIM histórico · quadro ${idx+1}/${model.resolvedSnapshots.length}`:`${Math.round(progress*100)}% · limite ETIM · quadro ${idx+1}/${model.resolvedSnapshots.length}`;updateOpenBadge();
  }
